...
function myUtilitiesTestFunction() {
  ...
  const sheetTimeZone = testSS.getSpreadsheetTimeZone();
  const timeX = Utilities.formatDate(new Date(), sheetTimeZone, "MM-dd-yyyy");
  Logger.log(`==> Utilities.formatDate ${timeX}`);
...
function getColumnValuesByName(sheet, columnName) {
  Logger.log(`==> getColumnValuesByName(${sheet.getName()}, ${columnName})`);
...
function getColumnIndexByName(sheet, columnName) {
  Logger.log(`==> getColumnIndexByName(${sheet.getName()}, ${columnName})`);
...
function setFormEditResponseUrls(sheet, myRow, myFormURL, setFormEditColumnIndex) {
  Logger.log(`==> setFormEditResponseUrls(${sheet.GetName()}, ${myRow}, ${myFormURL}, ${setFormEditColumnIndex})`);
...
/**
 * Iterates through the different sheets within a SpreadSheet and displays some overview information
 */
function iterateSheets(spreadsheet) {
    var sheets = spreadsheet.getSheets();
    Logger.log(`==> iterateSheets() of : ${spreadsheet.getName()}`);
  
  for (var i = 0; i < sheets.length; i++) {
    var sheet = sheets[i];
    Logger.log(`Sheet Name: ${sheet.getName()}`);
    Logger.log(`  Parent: ${sheet.getParent().getName()}`);
    Logger.log(`  Id: ${sheet.getSheetId()}`);
    Logger.log(`  Type: ${sheet.getType()}`);
